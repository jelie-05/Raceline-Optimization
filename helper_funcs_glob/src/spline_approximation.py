...
def spline_approximation(track: np.ndarray,
    ...
    print(f"no_points_reg_cl: {no_points_reg_cl}")
    print(f"shape of closest_t_glob_cl: {closest_t_glob_cl.shape}")
    w_tr_right_smoothed_cl = np.interp(np.linspace(0.0, 1.0, no_points_reg_cl), closest_t_glob_cl, w_tr_right_new_cl)
    print(f"shape of w_tr_right_smoothed_cl: {w_tr_right_smoothed_cl.shape}")
    ...
    return track_reg, closest_t_glob_cl
